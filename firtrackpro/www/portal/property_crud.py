...
def get_contacts(property_name: str):
    ...
    if frappe.db.has_column(DOC_PROPERTY, "property_contacts"):
        ...
        rows = frappe.get_all(
            ...
            filters={f"{DOC_PROPERTY.replace(' ','_').lower()}_parent": property_name} if frappe.db.has_column(f"{DOC_PROPERTY} Contact", f"{DOC_PROPERTY.replace(' ','_').lower()}_parent") else {"parent": property_name},
            fields=["name","parent","property_contact_contact","property_contact_role","property_contact_is_primary","property_contact_phone","property_contact_email"],
            limit=500,
        ...
        for r in rows:
            ...
            if cname and frappe.db.exists("Contact", cname):
                cdoc = frappe.db.get_value("Contact", cname, ["name","first_name","last_name","email_id","mobile_no"], as_dict=True) or {}
            out.append({
                "name": cname or r.get("name"),
                "full_name": " ".join([cdoc.get("first_name",""), cdoc.get("last_name","")]).strip() or r.get("property_contact_email") or r.get("property_contact_phone"),
...
def get_asset(name: str):
    ...
    fields = ["name","asset_label","asset_type","asset_zone","asset_status","asset_last_tested","asset_next_due"]
...
def save_asset(doc: dict | str):
    ...
    if d.get("name"):
        # update
        name = d["name"]
        vals = {k:v for k,v in d.items() if k not in ("doctype","name") and frappe.db.has_column(DOC_ASSET, k)}
        if vals: frappe.db.set_value(DOC_ASSET, name, vals)
        return {"name": name}
    # create
    d["doctype"] = DOC_ASSET
...
def delete_asset(name: str):
    if not name: frappe.throw(_("Missing name"))
...
def get_job(name: str):
    ...
    if not d: frappe.throw(_("Not found"))
    return d

@frappe.whitelist(methods=["POST"])
def save_job(doc: dict | str):
    d = _as_json(doc) or {}
    if not frappe.db.table_exists(DOC_JOB): frappe.throw(_("Jobs table not available"))
    if d.get("name"):
        name = d["name"]
        vals = {k:v for k,v in d.items() if k not in ("doctype","name") and frappe.db.has_column(DOC_JOB, k)}
        if vals: frappe.db.set_value(DOC_JOB, name, vals)
        return {"name": name}
    d["doctype"] = DOC_JOB
    ins = frappe.get_doc(d)
    ins.insert(ignore_permissions=True)
...
def delete_job(name: str):
    if not name: frappe.throw(_("Missing name"))
    if not frappe.db.table_exists(DOC_JOB): return "ok"
...
def save_quote(doc: dict | str):
    d = _as_json(doc) or {}
    if not d.get("name"): frappe.throw(_("Missing name"))
    vals = {k:v for k,v in d.items() if k not in ("name","doctype") and frappe.db.has_column(DOC_QUOTATION, k)}
    if vals: frappe.db.set_value(DOC_QUOTATION, d["name"], vals)
    return {"name": d["name"]}
...
def get_defect(name: str):
    if not name: frappe.throw(_("Missing name"))
    if not frappe.db.table_exists(DOC_DEFECT): frappe.throw(_("Defects table not available"))
...
def save_defect(doc: dict | str):
    ...
    if d.get("name"):
        name = d["name"]
        vals = {k:v for k,v in d.items() if k not in ("doctype","name") and frappe.db.has_column(DOC_DEFECT, k)}
        if vals: frappe.db.set_value(DOC_DEFECT, name, vals)
        return {"name": name}
...
def delete_defect(name: str):
    if not name: frappe.throw(_("Missing name"))
    if not frappe.db.table_exists(DOC_DEFECT): return "ok"
...
def link_search(doctype: str, q: str = "", limit: int = 10):
    ...
    for guess in ("asset_type_name", "customer_name", "item_name", "title", "description"):
        if frappe.db.has_column(doctype, guess):
            fields.append(guess)
            break
    rows = frappe.get_all(doctype, filters=cond or None, fields=fields, order_by="modified desc", limit=limit, ignore_permissions=1)
    # normalize 'label'
    for r in rows:
        if len(r) > 1:
