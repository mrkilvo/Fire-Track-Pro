...
def get_context(context):
	...
	if frappe.request.method == "POST":
		...
		if credit_limit is not None:
			cl = frappe.db.get_value("Customer Credit Limit", {"parent": doc.name}, "name")
			if cl:
		...
		if contact_link:
			contact = frappe.get_doc("Contact", contact_link)
			doc.primary_contact = contact.get("salutation", "") + " " + contact.get("first_name", "")
			...
			doc.primary_contact = ""
		credit_limit = frappe.db.get_value("Customer Credit Limit", {"parent": doc.name}, "credit_limit")
