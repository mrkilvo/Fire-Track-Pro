...
def _contacts_for_property(prop_name: str):
    ...
    out = []
    for c in contacts:
        fullname = (c.get("first_name") or "") + (
            (" " + c.get("last_name")) if c.get("last_name") else ""
        )
        out.append(
            {
                "name": fullname.strip(),
    ...
    return [
        {
            ...
            "next": (
                str(r["schedule_rule_next_occurrence"])
                if r.get("schedule_rule_next_occurrence")
                else ""
            ),
        }
        for r in rules
...
def get_context(context):
    ...
    if type_names:
        ...
        type_label_map = {
            t["name"]: (t.get("asset_type_label") or t["name"]) for t in trows
        }
