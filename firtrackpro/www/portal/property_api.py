...
def _coalesce_access_name(prop_doc):
    ...
    return frappe.db.get_value(
        DOC_PROPERTY_ACCESS, {"property_access_property": prop_doc.name}, "name"
    )
...
def get_property(name: str):
    ...
    if prop.property_customer:
        out["customer"] = frappe.db.get_value(DOC_CUSTOMER, prop.property_customer, ["customer_name"], as_dict=True)
...
def get_assets(property_name: str, limit: int = 100):
    return frappe.db.get_all(
        ...
        fields=["name", "asset_label", "asset_type", "asset_status", "asset_zone", "asset_last_tested", "asset_next_due"],
...
def get_defects(property_name: str, limit: int = 100):
    return frappe.db.get_all(
        ...
        fields=["name", "defect_severity", "defect_status", "defect_description", "defect_template", "defect_quotation", "modified"],
        order_by="modified desc",
        limit=limit,
    )
