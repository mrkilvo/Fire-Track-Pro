...
def get_property(name: str):
	...
	if d.get("property_customer") and frappe.db.exists("Customer", d["property_customer"]):
		cust = (
			frappe.db.get_value("Customer", d["property_customer"], ["name", "customer_name"], as_dict=True)
			or {}
		)
...
def get_assets(property_name: str):
	...
	return frappe.get_all(
		...
		if frappe.db.has_column(DOC_ASSET, "asset_property")
		else {"name": ["!=", ""]},
		fields=fields,
	...
	links = {
		...
		"related": {
			...
			"quotes": {
				...
				"count": _count(DOC_QUOTATION, "quote_property", name)
				if frappe.db.has_column(DOC_QUOTATION, "quote_property")
				else 0,
				"desk": f"/app/Quotation?quote_property={urllib.parse.quote(name)}"
				if frappe.db.has_column(DOC_QUOTATION, "quote_property")
				else f"/app/Quotation?customer={urllib.parse.quote(customer.get('name'))}"
				if customer
				else "/app/Quotation",
			},
		},
