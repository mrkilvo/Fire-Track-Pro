...
def _ensure_asset_type(label_or_code: str) -> str:
    ...
    # 1) Try exact name
    hit = frappe.get_all(
        "FT Asset Type", filters={"name": val}, fields=["name"], limit=1
    )
    if hit:
        return hit[0]["name"]
...
def _ensure_customer(customer_name: str) -> str:
    ...
    existing = frappe.get_all(
        "Customer", filters={"customer_name": customer_name}, fields=["name"], limit=1
    )
    if existing:
...
def create(
    ...
    if address_json:
        ...
        if isinstance(addr_obj, dict):
            ...
            if cust_docname:
                ad.append(
                    "links", {"link_doctype": "Customer", "link_name": cust_docname}
                )
            ad.insert(ignore_permissions=True)
            addr_docname = ad.name

    if not addr_docname and address_display:
        addr_docname = _create_address_from_display(
            property_name, address_display, cust_docname, lat, lng
        )
...
def save_address(
    ...
    if prop.property_customer and not any(
        l.link_doctype == "Customer" and l.link_name == prop.property_customer
        for l in (ad.links or [])
    ):
        ad.append(
            "links", {"link_doctype": "Customer", "link_name": prop.property_customer}
        )
    if ad.is_new():
        ad.insert(ignore_permissions=True)
    else:
...
def upsert_access(
    ...
    keysafe_code = (
        data.get("keysafe_code")
        if data.get("keysafe_code") is not None
        else keysafe_code
    )
    gate_code = (
        data.get("gate_code") if data.get("gate_code") is not None else gate_code
    )
...
def add_schedule_rule(
    ...
    return {
        ...
        "schedule_rule": {
            ...
            "next": (
                str(doc.schedule_rule_next_occurrence)
                if doc.schedule_rule_next_occurrence
                else ""
            ),
        },
    }
...
def add_asset(
    ...
    asset_status = _norm_asset_status(
        data.get("asset_status", asset_status) or "Active"
    )
    ...
    return {
        ...
        "asset": {
            "name": doc.name,
            "type": frappe.get_value(
                "FT Asset Type", doc.asset_type, "asset_type_label"
            )
            or doc.asset_type,
...
def get_asset(name: str):
    ...
    zone_name = (
        getattr(a, "asset_zone", "") if _has_field("FT Asset", "asset_zone") else ""
    )
    if zone_name:
        z = frappe.get_all(
            "FT Zone",
    ...
    return {
        ...
        "install_date": (
            str(gf("asset_install_date")) if gf("asset_install_date") else ""
        ),
...
def add_credential(
    ...
    if not table_field:
        frappe.throw(
            _(
                "FT Property missing child table 'property_credentials' (FT Property Credential Requirement)."
            )
        )
