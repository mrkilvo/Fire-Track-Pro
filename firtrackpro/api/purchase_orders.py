...
ALLOWED_ROLES = {"Administrator", "System Manager", "Member", "Accounts User", "Purchase User", "Purchase Manager"}
...
@frappe.whitelist()
def list_purchase_orders(q=None, status="all", date_from=None, date_to=None, limit_start=0, limit=50, supplier=None):
    ...
    try:
        if or_filters:
            total = len(frappe.get_all("Purchase Order", filters=filters, or_filters=or_filters, fields=["name"], limit=10000, ignore_permissions=1))
        else:
