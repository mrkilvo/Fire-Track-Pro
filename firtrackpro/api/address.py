...
def search(q=None, limit=7, country="AU"):
	...
	cache_key = "addr:nominatim:" + hashlib.sha1(json.dumps(params, sort_keys=True).encode()).hexdigest()
	...
	try:
		res = make_get_request("https://nominatim.openstreetmap.org/search", headers=headers, params=params)
	except Exception:
		try:
			import requests
